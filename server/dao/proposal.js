...
const $sqlQuery = require('./sqlCRUD').proposal;
const {
    getUuid,
    writeFile
} = require('../utils/utils');
...
module.exports = {
    ...
    getByPage(params) {
        console.log(params)
        let sqlStr = `select * from proposal`;
        let temp = {
            ...params
        };
        delete temp.pageNum;
    ...
    // 计算总数
    getSum(params) {
        let sqlStr = `select count(*) from proposal`;
        let temp = {
            ...params
        };
        delete temp.pageNum;
    ...
    getByProposalled(params) {
        let sqlStr = `select count(*) from proposal where proposal_time between '${params.startTime}' and '${params.endTime}'`;
