...
const $sqlQuery = require('./sqlCRUD').midcheck;
const {
    getUuid,
    writeFile
} = require('../utils/utils');
...
module.exports = {
    // 分页查询
    getByPage(params) {
        let sqlStr = `select * from midcheck`;
        let temp = {
            ...params
        };
        delete temp.pageNum;
    ...
    // 计算总数
    getSum(params) {
        let sqlStr = `select count(*) from midcheck`;
        let temp = {
            ...params
        };
        delete temp.pageNum;
    ...
    getByMidchecked(params) {
        let sqlStr = `select count(*) from midcheck where midcheck_time between '${params.startTime}' and '${params.endTime}'`;
