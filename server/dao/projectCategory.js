...
const $sqlQuery = require('./sqlCRUD').projectCategory;
...
module.exports = {
    ...
    getByPage(params) {
        console.log(params)
        let sqlStr = `select * from project_category`;
        let temp = {
            ...params
        };
        delete temp.pageNum;
    ...
    // 计算总数
    getSum(params) {
        let sqlStr = `select count(*) from project_category`;
        let temp = {
            ...params
        };
        delete temp.pageNum;
    ...
    save(obj) {
        if (!obj.Id)
            return _.query($sqlQuery.add, [getUuid(), obj.name, obj.description]);
        else {
            let params = {
                ...obj,
            };
