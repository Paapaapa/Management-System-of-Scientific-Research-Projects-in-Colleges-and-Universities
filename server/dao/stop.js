...
const $sqlQuery = require('./sqlCRUD').stop;
...
module.exports = {
    ...
    // 计算总数
    getSum(params) {
        let sqlStr = `select count(*) from stop`;
        let temp = {
            ...params
        };
        delete temp.pageNum;
    ...
    save(obj) {
        if (!obj.Id)
            return _.query($sqlQuery.add, [getUuid(), obj.project_id, obj.reason,obj.principal_id]);
        else {
            let params = {
                ...obj,
            };
