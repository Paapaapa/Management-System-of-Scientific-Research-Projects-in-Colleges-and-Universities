...
const $sqlQuery = require('./sqlCRUD').fruit;
const {
    getUuid,
    writeFile
} = require('../utils/utils');
...
module.exports = {
    ...
    // 计算总数
    getSum(params) {
        let sqlStr = `select count(*) from fruit`;
        let temp = {
            ...params
        };
        delete temp.pageNum;
    ...
    save(req) {
        ...
        if (!req.body.Id) {
            ...
            return _.query($sqlQuery.add, [Id, project_id, category_id, description, principal_id, file_path ? file_path : null])
    ...
    getByConfirmed(params) {
        let sqlStr = `select count(*) from fruit where confirm_time between '${params.startTime}' and '${params.endTime}'`;
